...
# Dynamic import for main app and routes
import importlib.util

main_path = Path(__file__).parent.parent / "main.py"
main_spec = importlib.util.spec_from_file_location("test_main", main_path)
main_module = importlib.util.module_from_spec(main_spec)
sys.modules['test_main'] = main_module
main_spec.loader.exec_module(main_module)

routes_path = Path(__file__).parent.parent / "api" / "routes.py"
routes_spec = importlib.util.spec_from_file_location("test_routes", routes_path)
routes_module = importlib.util.module_from_spec(routes_spec)
sys.modules['test_routes'] = routes_module
...
models_spec = importlib.util.spec_from_file_location("test_models", models_path)
...
def setup_clients(mock_k8s_client, mock_cloud_run_client, mock_actions_logger):
    """Setup mock clients before each test."""
    # Import the actual routes module from main
    import importlib
    if 'test_routes' in sys.modules:
        routes = sys.modules['test_routes']
        # Set global clients in routes module
        routes._k8s_client = mock_k8s_client
        routes._cloud_run_client = mock_cloud_run_client
        routes._actions_logger = mock_actions_logger
    ...
    if 'test_routes' in sys.modules:
        ...
        routes._cloud_run_client = None
        routes._actions_logger = None
    routes_module._actions_logger = None
